...
\author{%
	...
	\thanks{%
		...
		He is affiliated with the CRIStAL joint research group at the University of Lille,
...
indicate tensors. Uppercase letters $K$ represent fixed scalars and lowercas
...
\begin{equation}
	...
	\mat{V}_k\mat{\Lambda}\mat{V}_k^\intercal\text{.}
\end{equation}
...
\end{align}
with $\ten{E}^{(0)}=\ten{X}$.
Resulting core tensors can be vectorized and concatenated
\begin{equation}
	\mat{g}
...
Differences in classification score between two decoders for a dataset
...
\begin{figure}[t]
	...
	\caption{%
		...
		dimension.
		Classification score (\acs{rocauc}) increases with $B$, while reconstruction
...
We believe that \ac{bttda} could gain ground on \ac{mi} state of the art if problems
...
can discover more discriminant information over \ac{hoda}, improving classification
performance.
...
extracting further blocks now deteriorates performance.
