...
\ac{eeg} data (calibration) and applying the classifier to unseen \ac{eeg} data
(operation).
...
Due to their multichannel time series nature, \ac{eeg} or other functional
...
Tensor methods often decompose an tensor into a lower dimensional
core tensor and a set of factor matrices.
Most approaches adhere to either the Tucker structure or the PARAFAC
structure.
...
tensor decomposition and approximation in the Tucker.
...
of a scalar and per-mode factor vectors,
equivalent to a Tucker structured decomposition with all core elements
off the hyperdiagonal set to 0, as shown in \cref{fig:bttda/sparse}.
Unsupervised PARAFAC decomposition is performed through the Canonical Polyadic
Decomposition~\cite{Hitchcock1927,Nazarpour2006}.
These methods can used as feature extraction methods with the flattened core
...
Nevertheless, the common Tucker or \ac{parafac} structures might still fail to
...
solutions conforming to the Tucker or \ac{parafac} structure and their hyperparameters.
...
However, increased flexibility comes with a cost of more hyperparameters, as now
both the number of block terms and the dimension of each term need to be specified.
The unsupervised \ac{btd}~\cite{DeLathauwer2008,DeLathauwer2008a,DeLathauwer2008b,Rontogiannis2021} extracts the block core tensors

\subsection{Supervised tensor decompositions for \ac{bci}}

If tensor decompositions are not full rank, the Tucker, \ac{parafac} and block-term
structures are not unique and can be obtained by optimizing criteria other than
the supervised methods described above.
Given the low signal-to-noise ratio and specific, task-related output of \ac{bci}
applications, supervised feature extraction and machine learning is
favored~\cite{Lotte2018} over the unsupervised techniques
A decomposition for classification should ideally optimize
the class discriminability in the resulting core tensors containing the
extracted features.
In this philosophy, Tucker-structured decomposition can also be achieved
using \ac{hoda}~\cite{Yan2005,Phan2010,Froelich2018}, by optimizing class
...
Recent work proposes optimized objective functions or
regularizations~\cite{JamshidiIdaji2017,Jorajuria2022,Aghili2023}.
\Ac{parafac}-structured discriminant tensor decompositions have equally been appliedn
for instance through manifold optimization~\cite{Froelich2018}.
However, it is not trivial if the Tucker or \ac{parafac}
structure is the most suited representation for discriminatory neural data
for classification in \ac{bci} decoding.

More recent work adopts more flexible structures to improve \ac{bci}
performance
\Ac{hopls}~\cite{Zhao2012,Camarrone2018} and \ac{bttr}~\cite{Faes2022,Faes2022a}
achieve promising results for regression tasks.
\ac{bttr} has also been adapted into the classification variant
...
Thus, the method cannot be extended to multi-class settings.
Furthermore, the core structures of these methods are still
...
decomposition is restricted to terms with dimension $(R_1,R_2,1)$ with mode 3
...
complex discriminant neural data in a sparse way, additionally yielding a regularization effect.
Feature extraction with multiple parsimonious discriminant block terms with low dimension
might increase classification performance over a single \ac{hoda} block which
requires a high dimension to capture discriminant information, and by doing
so extracts too many irrelevant features.
From a complementary viewpoint:
if \ac{hoda} with a well-chosen reduced dimension extracts some discriminant
features, it is likely that it does not retrieve extract all useful
...
Analogous to \ac{hosvd}, \ac{hoda} decomposition yields a dense latent
tensor $\ten{G}$ and set of projection matrices (weights) $\mat{U}_k$ to ensure uniqueness.
...
\end{equation}
solved through the backward HODA algorithm.
...
\end{equation}
Subsequently, a new projection matrix $\mat{V}_k$ is found analogous to Linear
Discriminant Analysis using the partial within-class scatter matrix:
\begin{equation}
...
\end{equation}
which is solved $R_k$ leading eigenvectors,
...
and $\mat{U}_k$ obtained in the previous iteration.
...
\end{equation}
The iterative process halts when
each $\mat{U}_k$ update drops below a predetermined threshold $\epsilon$ or
after predetermined $I_\text{max}$ iterations.
The full procedure is summarized in \cref{alg:HODA}.
\begin{algorithm}
...
Next, these projections extract latent tensors from the training tensors, which
are then reshaped (\emph{vectorized}) into feature vectors
$\mat{g} =  \vect(\ten{G})$ and used to train a decision classifier in
conjunction with the class labels.
In operation, projections learned from training data are
used to extract latent tensors from unseen test data
and are also vectorized to pass on to the trained decision
classifier.

To avoid overfitting and improve performance in low sample size settings,
HODA was regularized at each iteration by shrinking the partial
within-class scatter matrices~\cite{Phan2010} with a shrinkage factor
$\alpha_k$, such that the eigenvalue problem becomes
\begin{equation}
	\mat{S}_b^{(-k)} -
	\varphi\left[\left(1-\alpha_k\right)\mat{S}_{-k,\text{w}}+\alpha_k\mat{I}\right] =
	\mat{V}_k\mat{\Lambda}\mat{V}_k^\intercal
\end{equation}
As in Linear Discriminant Analysis, data-driven estimates of
$\alpha_k$~\cite{Jorajuria2022} can be estimated, e.g, by the Ledoit-Wolf procedure~\cite{Ledoit2003}.
...
but \ac{bttda} relies on reconstruction with minimal reconstruction error.

\Cref{eq:HODA-forward} gives a straightforward and computationally efficient
\ac{hoda} forward model candidate, visualized in \cref{fig:hoda} (right):
\begin{equation}
...
the noise term and extracted latent tensors:
\begin{equation}
	\left\{\mat{A}^*\right\}
...
\end{equation}
At every iteration, this is solved directly by ordinary least squares.
...
\Cref{alg:HODA-fw} lists the full procedure.
\begin{algorithm}
...
If the latent tensors $\ten{G}$ after \ac{hoda} backward projection does not
achieve perfect class separation, the error term $\ten{E}$ in
\cref{eq:HODA-forward} contains further discriminative information.
This can be exploited to improve classifier performance.
...
We thus extend the \ac{hoda} to \acf{bttda}, extracting multiple discriminative
latent blocks, such that its forward model adheres to the block-term tensor
structure:
\begin{equation}
...
\end{equation}
for classification similarly to \ac{hoda}.
...
which replaces the block dimensions $\{(R_1^{(b)}, R_2^{(b)}, \ldots,	R_K^{(b)})\}_b^B$.
...
each block optimizes the discriminability criterion independently from the others.
To mitigate this, extracted features are first decorrelated and scaled using
whitening \ac{pca}.
Relevant \ac{pca} components are identified using their univariate Fisher score
$\phi(i)$ calculated as
\begin{equation}
	\phi(i) = \frac
...
the \acf{erp} and \acf{mi} paradigms using the openly available \ac{moabb}
benchmark datasets (version 1.2.0)~\cite{Aristimunha2023} allowing fair comparison
across classifiers.
...
For \ac{erp} decoding, these included Riemannian Geometry-based
classifiers ERPCov+MDM, ERPCovSVD+MDM, XDAWNCov+MDM, XDAWNCov+TS+SVM and the linear
classifier XDAWN+LDA.
For \ac{mi}, Riemannian classifiers ACM+TS+SVM, FgMDM, TS+EL, and the deep learning classifiers EEGTCNet and ShallowConvNet were used.
...
and 24 Hz.
Signals were then cut into epochs starting from stimulus onset with a
...
\cref{tab:results/erp/score}, BTTDA+LDA (avg. \ac{rocauc}: 91.25$\pm$6.77\%) outperforms PARAFAC+LDA
(90.94$\pm$6.90\%), and both in turn outperform HODA+LDA (88.89$\pm$7.04\%).
...
\end{table*}
BTTDA+LDA outperforms state-of-the-art decoder XDAWNCov+TS+SVM in 8 out of 14
datasets, resulting in a moderate increase in grand average \ac{rocauc}
($91.25 > 90.82$).


\subsection{Motor Imagery}

\Ac{mi} discriminatory information is encoded in the \ac{eeg} data as
\acp{ersd}.
Contrary to time-domain analysis for \acp{erp}, \acp{ersd} are
expressed in changes in time-frequency power.
After time-frequency transformation, \ac{eeg} forms a third-order tensors, with $K=3$ modes
respectively representing channels, frequencies, and time bins.

\Ac{eeg} signals in the \ac{mi} datasets listed in \cref{tab:moabb}
are first processed using the \ac{moabb} motor imagery pipeline.
They were recorded at the sample rate given
by \cref{tab:moabb} and band-pass filtered between 8 Hz
and 32 Hz.
The signals were then cut into epochs from stimulus onset to a
dataset-specific length given by \cref{tab:moabb}.
Custom time-frequency postprocessing extracted the magnitude of the complex
Morlet-wavelet transform with 17 logarithmically spaced frequencies from 8 Hz
to 32 Hz and a varying number of cycles logarithmically spaced from 4 to 16.
...
BTTDA+LDA (avg.\ accuracy: $64.52\pm12.23$\%)
...
\begin{table*}
	...
	\label{tab:mi-score}%
\end{table*}
All of HODA+LDA (avg.\ accuracy $61.00\pm11.11$), PARAFACDA+LDA
...
\Cref{fig:blocks} (left) reports a selection of cross-validated within-session
...
is by definition a lossless decomposition of the input dataset.
...
In general, the reduced dimension should be high when $b$ is low.
This can be reduced with $b>1$ to a point where the total reduced
dimension is lower and performance is higher.

Additionally, \cref{fig:blocks} (right) shows forward modeling effectiveness
...
The activation patterns of the first two blocks disentangle this in effects
that can be related to \ac{erp} literature in the context of the
...
This improvement results in state-of-the-art decoding performance for \ac{erp}
datasets in the \ac{moabb} benchmark, but subpar scores for \ac{mi}.
...
interpretation of the signal components contributing to classification.
While the backward model weights are hard to interpret~\cite{Haufe2014}, the
activation patterns and contrasts after forward projection can separate and
reveal patterns of task-related physiological activity in neural data as shown
by qualitative for both \ac{erp} and \ac{mi} decoding.
...
eventually overpasses the \ac{hoda} solution as shown by~\cref{fig:blocks}.
...
hyperparameters to tune, expanding the solution space to find performant settings but
increasing time complexity.
The most computationally expensive step in \ac{hoda} feature extraction is
the multi-mode products $\ten{G}\left(n\right) = \ten{X}\left(n\right)\mpr{\mat{U}}{-k}\ \forall n$, with
complexity $\mathcal{O}\left(NKD^{K+1}\right)$, or,
...
Including hyperparameter optimization, this gives
\begin{equation}
...
does not guarantee that coherent projection are grouped within one block and
can result in heavily correlated features across blocks.
...
with applications in \acf{decoding}, particularly for \acf{erp} decoding.
...
domains.
The resulting model inherently provides interpretable projections.
Moving from the rigid Tucker tensor structure techniques to the more flexible
block-term tensor structure shifts the focus of tensor discriminant analysis
