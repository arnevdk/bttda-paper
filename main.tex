...
\author{%
	...
	Hakim Si-Mohammed,
	Fran\c{c}ois Cabestaing, \IEEEmembership{Senior Member, IEEE}
	Marc M. Van Hulle, \IEEEmembership{Senior Member, IEEE}
	\thanks{%
	...
	\thanks{%
		Arne Van Den Kerchove is with the Department of Neurosciences, KU Leuven,
		BE-3000 Leuven, Belgium and the CRIStAL joint research group (UMR CNRS 9189)
		co-administered by the University of Lille,
		CNRS, and Centrale Lille, France (e-mail: arne.vandenkerchove@kuleuven.be).
	}
	\thanks{%
		...
		joint research group  (UMR CNRS 9189) co-administered by the University of Lille, CNRS, and Centrale
		Lille, FR-59655 Villeneuve d'Ascq, France.
	}
	\thanks{%
		Marc M. Van Hulle is with the Department of Neurosciences, KU Leuven.
...
%\Acp{bci}
\IEEEPARstart{B}{rain}-computer interfaces (BCIs)
have the potential to bypass
...
the recorded signal, coupling it to output or actions, which often gives rise to classification problems~\cite{Lotte2018}.
...
information in both the spatial and the temporal domains.
Preprocessing transformations can further expand these data into additional
analytic domains.
...
These high-dimensional datasets are usually flattened
into a set of sample vectors, stripping them of their inherent structure.
...
high dimensionality.
Tensor methods are machine learning or dimensionality reduction techniques that treat
each tensor mode separately, reducing a given problem to partial per-mode problems.

Tensor methods often decompose a tensor into a lower-dimensional
core tensor and a set of factor matrices, adhering to either the Tucker
...
These decompositions can extract feature vectors from input data using the
flattened core tensors, which can subsequently be classified to predict class labels, most
...
\begin{figure*}[t]
	...
	\hfill%
	\caption{%
		A tensor decomposition finds a core tensor and factor matrices
		...
		allows blocks with variable tensor dimensions instead of fixed rank-1 terms.
...
Given the low signal-to-noise ratio and specific, task-related output in \ac{bci}
...
regularization~\cite{Li2010,JamshidiIdaji2017,Jorajuria2022,Aghili2023}.
\Ac{parafac}-structured discriminant tensor decomposition has also been
applied~\cite{Froelich2018}, but it is not trivial whether Tucker or \ac{parafac}
is most suited for neural data in \ac{bci} decoding.
...
multilinear spectral filter terms applied to \ac{mi}, yet their
decomposition is also restricted to terms with dimension $(R_1,R_2,1)$ with the
frequency domain as mode 3.
...
complex discriminant neural data in a sparse way, yielding a regularizing effect.
...
Could \ac{hoda}, therefore, not sequentially be applied to extract additional discriminant
Tucker structured terms -- potentially with a lower dimension -- as long as decoding
performance improves?

We implement a novel supervised feature
extraction method titled \ac{bttda}%
\footnote{%
	The source code of the
	methods introduced in this work is available at
	\url{https://github.com/arnevdk/bttda}.
}, a generalization of \ac{hoda}.
...
2) This allows us to introduce \ac{bttda} as a state-of-the-art \ac{bci}
...
indicate tensors. Uppercase letters $K$ represent fixed scalars and lowercase
letters $k$ variable scalars.
...
which project a given $\ten{X}$ to a core tensor
...
\begin{figure*}
	...
	\caption{%
		...
		discriminability between classes is achieved in the projected latent core tensors
		$\ten{G}$ with reduced dimensions $(R_1,R_2,R_3)$.
		\subref{fig:hoda/fw} By calculating activation patterns $\mat{A}_k$, the original tensor
		$\ten{X}$ can approximately be reconstructed from projected latent core tensor $\ten{G}$.
...
Analogous to \ac{hosvd}, \ac{hoda} decomposes $\ten{X}$ in a dense core
tensor $\ten{G}$ and a set of orthogonal projection matrices (weights) $\mat{U}_k$ to ensure uniqueness.
...
Discriminability is defined in the Fisher sense, maximizing the Fisher ratio $\phi$
\begin{equation}
	...
	\label{eq:fisher}
\end{equation}
for $C$ classes, each with $N_c$ samples.
$\bar{\ten{G}}(c)$ is the mean of
core tensors of class $c$, and $\bar{\bar{\ten{G}}}$ the mean of
...
\begin{equation}
	\left\{\mat{U}^*\right\} =  \argmax_{\{\mat{U}\}}\phi\left(\left\{\mat{U}\right\}\right)\text{,}
\end{equation}
...
a partial core tensor
\begin{equation}
	\ten{G}_{-k}=\ten{X}\mmprs{\mat{U}}{k}\text{.}
\end{equation}
...
\begin{equation}
	\mat{S}_{-k,\text{w}} = \sum_n^N\tilde{\mat{G}}_{-k,k}(n)\cdot\tilde{\mat{G}}_{-k,k}^\intercal(n)\text{,}
\end{equation}
with $\tilde{\ten{G}}_{-k}(n) = \ten{G}_{-k}(n) - \bar{\ten{G}}_{-k}(c_n)$,
and the partial between-class scatter matrix
\begin{equation}
	\mat{S}_{-k,\text{b}} =
	\sum_c^CN_c\tilde{\bar{\mat{G}}}_{-k,k}(c)\cdot\tilde{\bar{\mat{G}}}_{-k,k}^\intercal(c)\text{,}
\end{equation}
...
\begin{equation}
	...
	\text{,}
\end{equation}
by finding the $R_k$ leading eigenvectors of
\begin{equation}
	\mat{V}_k\mat{V}_k^\intercal\mat{S}_{k,\text{t}}\mat{V}_k\mat{V}_k^\intercal
	= \mat{U}_k\mat{\Lambda}\mat{U}_k^\intercal\text{.}
\end{equation}
The iterative process halts when each $\mat{U}_k$ update drops below a
predetermined threshold $\epsilon$ or number of iterations  $I_\text{max}$.
\begin{algorithm}
...
$\mat{g} =  \vect(\ten{G})$ and used together with the class labels to train a decision classifier.
In operation, the learned projections extract core tensors from unseen test data,
...
\begin{equation}
	\mat{S}_{-k,\text{b}} -
	\varphi\left[
	\left(1-\alpha_k\right)\mat{S}_{-k,\text{w}}
	+\alpha_k\frac{\tr\left(\mat{S}_{-k,\text{w}}\right)}{D_k}\mat{I}
	\right]
	= \mat{V}_k\mat{\Lambda}\mat{V}_k^\intercal
	\text{.}
\end{equation}
...
\ac{hoda} forward model candidate
\begin{equation}
...
reconstructed tensor $\hat{\ten{X}}$, and error term $\ten{E}$, visualized in
\cref{fig:hoda/fw}.
...
\begin{equation}
	...
	\sum_n^N\left[\mat{X}_k(n) -
		\mat{A}_k\left(\ten{G}(n)\mmprs{\mat{A}}{k}\right)_k\right]^2\text{,}
\end{equation}
...
model halts after $I_\text{max}$ iterations or when the update of each
...
block-term tensor model \ac{bttda}.
If extracted features $\left\{\vect\left(\ten{G}(n)\right)\right\}_n^N$ after \ac{hoda} backward projection do not
...
Additional features are extracted from $\ten{E} = \ten{X} -
...
latent core blocks such that its forward model adheres to block-term tensor
structure
\begin{equation}
	\ten{X} = \sum_b^B\ten{G}^{(b)}\mmpr{\mat{A}^{(b)}} + \ten{E}
	\label{eq:BTTDA-forward}
\end{equation}
for $B$ extracted core tensors $\ten{G}^{(b)}$ and residual error term
...
\begin{figure*}[t]
	...
	\caption[A forward model for \acs{bttda}.]{A forward model for \acf{bttda}.
		...
		than \ac{hoda} by iteratively finding core tensors $\ten{G}^{(b)}$ in using
		deflation.
		The \ac{hoda} backward projection is first applied. Next, the
		input data are reconstructed via the HODA forward model and the
...
is required to compute latent tensors $\ten{G}^{(b)}$ given $\ten{X}$ for
feature extraction.
...
\end{algorithm}
The core tensor corresponding to each block $b$ is extracted using the
...
\end{align}
with $\ten{E}^{(0)}=\ten{X}$.
Resulting core tensors can be vectorized and concatenated as
\begin{equation}
	\mat{g}
...
For $0 < \theta < 1$, the dimensions of block $b$ are be determined as by
\textcite{Phan2010}:
$R_k$ are chosen as the number of components needed to explain a
proportion $\theta$ of the corresponding mode $k$ variability of the input data.
For each block in \ac{bttda}, this is determined by the eigenvalues of the per-mode
total scatter matrix of tensor $\ten{E}^{(b-1)}$,
\begin{equation}
...
framework and datasets (version 1.2.0)~\cite{Aristimunha2023},
allowing fair comparison across classifiers.
\Cref{tab:app/moabb-datasets} lists details about these datasets.
\Ac{erp} decoding distinguishes target from non-target \acp{erp},
while \ac{mi} decoding distinguishes different imagined or performed
limb movements.
...
\ac{moabb}.
Average performance scores are balanced over datasets by taking the mean of
...
Hyperparameter candidates $\theta \in \left\{0, 0.1, 0.2, \ldots 1\right\}$
...
Other hyperparameters were set to $\epsilon=\num{1e-6}$ and $I_\text{max}=128$,
and $\mat{U}_k^{(b)}$ were initialized using the \ac{svd} of $\ten{E}^{(b-1)}_k$.
Per-dataset pairwise differences in decoder classification score
were verified with one-sided Wilcoxon rank-sum tests on the
...
over the \ac{erp} and \ac{mi} datasets separately, and effect size was determined
as the \ac{smd}.
...
We refer to \textcite{Chevallier2024} for implementation details
and references for these methods.

\subsection{Event-Related Potentials}
\Acfp{erp} are spatiotemporal features, with each epoch a second-order tensor (matrix)
with $K=2$ modes representing \ac{eeg} channels and time samples.
\Ac{eeg} signals are first processed according to the \ac{moabb} framework by
band-pass filtering between 1 Hz
and 24 Hz, and cutting epochs from stimulus onset with corresponding
dataset-specific length (see \cref{tab:app/moabb-datasets}).
...
\begin{table*}
	...
	\caption{Cross-validated withing session \Ac{rocauc} (\%) for HODA+LDA and
		our proposed decoders
...
time-frequency power, forming a third-order tensor with $K=3$ modes
...
Morlet wavelet magnitudes for 17 logarithmically spaced frequencies (8 - 32 Hz)
...
In line with the results presented by~\textcite{Chevallier2024}, only the first three classes per dataset were
used.
Based on grand-average accuracy score reported in \cref{tab:mi-score},
...
\begin{table*}
	...
	\caption{Cross-validated within-session classification accuracy (\%)
	...
	\label{tab:mi-score}%
\end{table*}
All of HODA+LDA, PARAFACDA+LDA, and BTTDA+LDA score substantially lower than
state-of-the-art decoder ACM+TS+SVM (75.77$\pm$11.12).
...
\ac{rocauc} scores in function of the number of blocks ($b$) and hyperparameter
...
\begin{figure}[t]
	...
	\caption{%
		...
		dimension.
		Classification score  increases with $B$, while reconstruction
		error decreases.
		Eventually, overfitting occurs and classification score drops or plateaus,
		depending on feature selection effectiveness.
	}\label{fig:blocks}
\end{figure}
BTTDA+LDA is equivalent to PARAFACDA+LDA and HODA+LDA when $\theta=0$ or $b=1$ respectively.
...
at $b=3$ (avg.\ \ac{rocauc} 85.74\% and 85.71\% respectively), with lower
reduced dimension than the high optimal dimension for \ac{hoda}.
Eventually, BTTDA+LDA reaches optimal overall \ac{rocauc} at
...
The following qualitative analysis reveals forward modeling interpretability by
...
for \ac{erp} analysis and AlexandreMotorImagery for \ac{mi}.
...
Each model was then retrained with these hyperparameters on all combined
subject data and reconstructed condition contrasts were generated as
...
\Ac{mi} contrasts in the time-frequency domain show positive values for
...
increase of 2.36\%pt.\ for \ac{erp} datasets and 2.75\%pt.\ for \ac{mi}.
...
Low \ac{mi} performance can partially be attributed to low
\ac{mi} performance of base model \ac{hoda}.
...
We believe that \ac{bttda} could gain ground on the \ac{mi} state of the art if the
problems hampering \ac{hoda} performance can be identified.
Hence, the following candidates should be investigated.
First, the current time-frequency transformation or corresponding parameters
might not be suited to capture the \ac{ersd} information necessary
for the evaluated \ac{moabb} classification tasks.
Second, standardized \ac{moabb} preprocessing for fair decoder comparison
...
multi-class problems, since reduced block dimensions might be too low
to separate the classes.
In general, \ac{mi} decoding might benefit from different transformation or
tensorization techniques.

Due to the inherent forward model, \ac{bttda} is intrinsically explainable,
allowing interpretation of signal components contributing to classification.
While backward weights are hard to interpret~\cite{Haufe2014},
activation patterns and contrasts after forward projection
reveal and separate patterns of task-related physiological activity in neural data, as shown
in \cref{fig:results/interpret}.
...
A point of care, however, arises from the deflation scheme:
Previous blocks might already have partially explained some effects, which warrants
inspecting these patterns together with the contribution of a given block.
...
can discover more discriminant information than \ac{hoda}, improving classification
performance.
While no proof is given here, we have shown that \ac{nmse} monotonically decreases
with $B$, suggesting all signal variation will eventually be explained while
extracting maximally discriminant features.
However, overfitting eventually leads to diminishing generalization returns and
extracting excessive blocks will deteriorate performance.
On the other hand, \cref{fig:bttda/sparse} illustrate that block-term structured
ac{bttda} can be more parsimonious than Tucker-structured \ac{hoda}.
...
a benefit or a drawback depending on the required amount of regularization required and
...
eventually surpasses the \ac{hoda} solution as shown by~\cref{fig:blocks/score}.
...
its overall model of the input data scatter is constrained by these per-mode scatter
matrices as a Kronecker product $\mat{S}_{-1,\text{w}}\otimes \mat{S}_{-2,\text{w}}\otimes\cdots\otimes \mat{S}_{-K,\text{w}}$,
which assumes that data are drawn from a multilinear normal
distribution~\cite{Ohlson2013}.
Similar assumptions are made in \ac{erp} decoding algorithms such as
Spatial-Temporal Discriminant-Analysis~\cite{Zhang2013,GonzalezNavarro2017} and LCMV-beamforming
...
product. It is more accurately modeled as a sum of Kronecker
...
In summary, we conclude that there is an effective added value in iteratively extracting
...
hyperparameters, expanding the solution space to more performant settings, yet
increasing computation requirements.
The most compute-heavy \ac{hoda} step is the
multi-mode product $\ten{G}\left(n\right) = \ten{X}\left(n\right)\mpr{\mat{U}}{-k}\ \forall n$, with time complexity $\mathcal{O}\left(NKD^{K+1}\right)$.
\Ac{hoda} complexity is then $\mathcal{O}\left(I_\text{max}NK^2D^{K+1}\right)$
when iterating over the inner ($K$) and outer ($I_\text{max}$) loops.
With hyperparameter optimization, this gives
\begin{equation}
...
\Ac{bttda} increases this with a factor of $B_\text{max}$ to
\begin{equation}
	\mathcal{O}\left(\left|\Theta\right|FB_\text{max}I_\text{max}NK^2D^{K+1}\right)
\end{equation}
when efficiently using a grid search over $\theta$ combined with a line search over $B\leq B_\text{max}$.
Time complexity is derived in \cref{app:complexity}.
...
search space from $\textstyle{\left\{  \left(R_1^{(b)},R_2^{(b)},\ldots,R_K^{(b)}\right)\right\}_b^B}$ to $\Theta$.
On the other hand, it constrains the reduced dimensions of each block to
a subset of all possible configurations, conflicting with the earlier
proposition of increased model flexibility.
Given total free choice of block dimensions, situations might occur where
\ac{bttda} offers little to no improvement over \ac{hoda}, or could perform worse.
Settings whit higher \ac{bttda} could equally occur.
...
carefully weigh computational demands against potential performance gains.

Finally, we note that \ac{bttda} or the accompanying model selection procedure
does not guarantee that coherent projections are grouped within one block;
...
Other examples include block, sparsity, pattern interpretability measures, minimal or maximal
...
\ac{bttda} can equally be applied to classification of other neuroimaging modalities
...
methods inspired by prior data properties or the impact of higher-order tensors
($K>3$), or regularize the underdetermined forward model.
...
\input{complexity_derivation.tex}

\subsection{\Ac{moabb} benchmarking datasets}
\begin{table}[H]
	...
	{\footnotesize%
		...
		T/c: number of trials per class,
		L: epoch length (s),
	...
	\caption{
		\Ac{moabb} benchmark datasets, adapted from \cite{Aristimunha2023} and
		\textcite{Chevallier2024}.
		\Ac{erp} datasets contain 2 classes.
		For \ac{mi} datasets, the first 3 classes were retained.
		\Ac{erp} dataset Sosulski2019 was omitted due to technical problems.
		\Ac{mi} dataset PhysionetMI was omitted due to high computational and storage demands.
	}
	\label{tab:app/moabb-datasets}
\end{table}

\printbibliography%
\vskip -2\baselineskip plus -1fil
\begin{IEEEbiographynophoto}{Arne Van Den Kerchove}
	received the B.Sc.\ degree in Informatics and the M.Sc.\ in Computer Science
	from KU Leuven, Belgium in 2017 and 2020 respectively.
	In 2024, he received a joint\ Ph.D. degree in Biomedical Sciences and Control
	Sciences \& Signal Processing from KU Leuven and the University of Lille, France.
	He is currently with the Computational Neuroscience Research Group of the
	Laboratory for Neuro- and Psychophysiology at KU Leuven and affiliated with
	the Lille Joint Research Center for Informatics, Signal Processing and Control Science
	(CRIStAL, UMR CNRS 9189).
	His research focuses on machine learning methods and applied experimental research
	for assistive brain-computer interfaces and cognitive decline screening.
\end{IEEEbiographynophoto}
\vskip -2\baselineskip plus -1fil
\begin{IEEEbiographynophoto}{Hakim Si-Mohammed}
	is Associate Professor (Maître de Conférences) at the University of Lille,
	France and a member of the BCI research group of the Lille Joint Research
	Center for Informatics, Signal Processing and Control Science (CRIStAL,
	UMR CNRS 9189).
	His research lies at the intersection of brain-computer interfaces and
	immersive technologies (Augmented and Virtual Reality).
	He received an Engineering degree and M.Sc. from ESI Algiers, Algeria, in 2016,
	and a Ph.D. from INSA Rennes and Inria Rennes, France, in 2019.
	He was awarded the Rennes 1 Foundation Ph.D. Prize in 2020 and is Deputy
	Director of the SCV Research Federation since 2026.
\end{IEEEbiographynophoto}
\vskip -2\baselineskip plus -1fil
\begin{IEEEbiographynophoto}{François Cabestaing}
	(Senior Member, IEEE) received the B.S. degree in Mathematics in 1987 from
	USTL, the M.Sc. degree in Engineering and Control Science in 1987 from ISEN,
	and the Ph.D. degree in Signal Processing and Control Science in 1991 from the
	University of Lille, France. He holds an Habilitation à Diriger des Recherches
	obtained in 2003 from ULille. He has been an associate professor at ULille since
	1992 and became full professor in 2006 in the same university. He is a
	researcher at the Lille Joint Research Center for Informatics, Signal
	Processing and Control Science (CRIStAL, UMR CNRS 9189), where he heads the
	BCI (Brain-Computer Interfaces) research team. He studies physiological signal
	processing techniques, mainly electroencephalogram, and classification techniques,
	particularly to estimate the mental states of brain-machine interface users.
	He contributed to the creation of the COllectif pour la Recherche
	Transdisciplinaire sur les Interfaces Cerveau-Ordinateur (CORTICO), a
	learned society of which he was president from 2017 to 2024.
\end{IEEEbiographynophoto}
\vskip -2\baselineskip plus -1fil
\begin{IEEEbiographynophoto}{Marc M. Van Hulle}(Senior Member, IEEE)
	received the B.S. degree in Economics and the
	MBA degree from Hasselt University,
	Belgium, in 1986 and 1991, respectively, and the
	M.Sc. degree in electrotechnical engineering and the
	Ph.D. degree in applied sciences from KU Leuven, Belgium.
	Prof. Van Hulle received the Doctor Technices degree from Queen Margrethe
	II of Denmark, in 2003, and Honorary Doctoral degrees from Brest State
	University (BrSU), Brest, Belarus, in 2009, Yerevan
	State Medical University, Yerevan, Armenia, in 2013, and the Aurel
	Vlaicu University of Arad, Romania, in 2022.
	He is currently a Full Professor at the Medical
	School, KU Leuven, where he heads the Computational Neuroscience Research Group of the
	Laboratory for Neuro- and Psychophysiology.
	Prof. Van Hulle is member of the Belgian Royal Academy
	of Medicine and the Spanish Royal National Academy of Pharmacy,
	Institute of Spain.
\end{IEEEbiographynophoto}
\end{document}
