...
\todo{dimensionality} respectively.
...
\begin{figure*}[t]
	...
	\caption{Normalized Mean Square Error (NMSE) (left), and difference in area
		under the receiver operating curve (ROC-AUC) for the training and validation
...
\section{Discussion}
\todo{bad results for MI dataset, might be due to tensorization, also explain
in function of validation curves why something works or not works over HODA and
over general}
...
%\todo{check if this is still true, also descrive Tikhonov regularization in forward model if kept}
