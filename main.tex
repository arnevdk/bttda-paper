...
This can result in high-dimensional datasets\todo{But usually flattened into
	vectors or matrices… so stripping the original data structure.}
This section is a bit bumpy, the term multiway neural data should be used at the onset…, yet with a certain degree of decoupling
...
methods.
Tensor methods consider each analysis domain (tensor \emph{mode})\todo{What is a mode in this context? We use dimension, feature, (multi)way, domain…
	Please check/streamline terminology.}\todo{consistently use mode instead of
	domain or dimensions} separately,
...
supervised feature extraction and machine learning techniques are often
of\todo{(Linear) Combinations of tensors dimensions?}
...
methods, such as Linear Discriminant Analysis\todo{Maybe introduce LDA and use
	it further like HODA, etc..} or beamforming.
...
feature extraction technique. For a set of $N$ $K^{th}$ order input\todo{Before
	not called “input”…} tensors
$\left\{\ten{X}(n)\in\mathbb{R}^{D_1\times D_2 \times \cdots \times
		D_K}\right\}_n^N$,\todo{Is “n” an index as it appears as an index (marked
	in yellow) although the series D_1… D_N does not feature a D_n…} \textsc{HODA} finds projection matrices $\mat{U_k}$ for each mode $k$
...
In neuroimaging\todo{Why only neuroimaging? It s conceptual…}, this is referred to as a \emph{forward model}.
...
informed by prior knowledge about source propagation\todo{???}, a forward model is a
...
obtained by the backward\todo{I am confused. Backward HODA allows to classify but the forward HODA model has the error term which can be used to further improve classification.
	A few lines later forward HODA is specified, but backward HODA is needed to
	produce tensor G. Forward HODA is not iterative but backward HODA is (cf.
	deflation scheme), right?} projection of \textsc{HODA} do not achieve perfect
...
until error term $\ten{\mathbfcal{E}}$ remains.\todo{Remains? It is present.}
...
While we can reasonably set $\alpha=0.05$\todo{p-value threshold?}, $B$ and the block ranks must be
...
Two ERP\todo{define} and two MI\todo{define} datasets were retained for their size to reduce computational
demand.\todo{?? for data set size, but at the onset high-dimensional data was
	targe}
...
$1\leq B\leq16$.
Additionally, we also introduce PARAFACDA+LDA\todo{PARAFACDA? I assume PARAFAC
	+ LDA…}\todo{make more clear, first introduce PARAFACDA}, which is the special case of
...
detectable in the time-frequency domain.\todo{But electrode space as well… left
	vs. right hand…}
...
$c=0.7*f$.
Features were extracted by taking the power\todo{You mean Re and Im parts
	squared and summed?} of the wavelet transformed data,
...
drawn from a multilinear\todo{A bit unusual term, multivariate…}\todo{cite} normal distribution.
...
step.\todo{Significantly improves over HODA as main contribution}
...
(second order tensors) and \todo{new performance MI},
