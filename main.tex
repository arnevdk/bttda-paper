...
While effective, other approached such as the PARAFAC structure employed in
...
Squares~\cite{Camarrone2018} or Block-Term Tensor Regression (BTTR)~\cite{Faes2022,Faes2022b}
BTTR has been adapted into a classification variant, named Block-Term
Tensor Classification (BTTC)~\cite{Camarrone2021}, but since features
...
Furthermore, structures employed in HOPLS, BTTR and
BTTC are still more constrained than what could be achieved with a
full block-term tensor structured decomposition optimized for discriminability,
since they rely on a low rank common subspace between the input and
classification labels and BTTC cannot be applied in multi-class settings.
...
\begin{enumerate*}[label={\arabic*)}]
	\item first, we develop a forward model for HODA to reconstruct a
	...
	\item This allows us to introduce a state-of-the-art BCI feature extraction
        method based on the block-term tensor structure, named Block-Term Tensor Discriminant Analysis
	      (BTTDA).
      \item Finally we evaluate a BCI decoder based on BTTDA and its special
...
orthogonal matrices, by a per mode Singular Value Decomposition (SVD),
...
As a prerequisite for our proposed BTTDA model, we must first find a
...
We thus extend the HODA feature extraction scheme with backward an
...
(BTTDA).
BTTDA finds multiple discriminative blocks, such that its a forward
